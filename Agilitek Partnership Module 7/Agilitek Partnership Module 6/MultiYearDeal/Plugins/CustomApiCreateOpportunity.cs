...
{
    public class CustomApiCreateOpportunity : IPlugin
    {
        enum ats_type_opp
        {
        ...
        public void Process(IOrganizationService service, ITracingService tracingService = null, IPluginExecutionContext pluginContext = null, EntityReference agreementRefER = null, string inputActionNameER = null, int batchCount = 0)
        {
            ...
            try
            {
                ...
                string inputActionName = inputActionNameER;

                if (pluginContext != null &&
                    pluginContext.InputParameters.Contains("InputAction") &&
                    pluginContext.InputParameters["InputAction"] != null)
                {
                    inputActionName = pluginContext.InputParameters["InputAction"].ToString();
                }
                GetOpportunityProducts getOpportunityProducts = new GetOpportunityProducts();

                if (inputActionName == "OLIBatching")
                {
                    
                    //Getting the values from the context
                    string oliBatchingValue = pluginContext.InputParameters.Contains("InputBatchingProductsToCopy") && pluginContext.InputParameters["InputBatchingProductsToCopy"] != null
                        ? pluginContext.InputParameters["InputBatchingProductsToCopy"].ToString()
                        : string.Empty;
                    //List<Entity> BatchingProductsToCopy = !string.IsNullOrEmpty(oliBatchingValue)
                    //                                    ? JsonConvert.DeserializeObject<List<Entity>>(oliBatchingValue)
                    //                                    : new List<Entity>();

                    //Read IDs instead of Entity list
                    List<Guid> batchingIds = !string.IsNullOrWhiteSpace(oliBatchingValue)? JsonConvert.DeserializeObject<List<Guid>>(oliBatchingValue): new List<Guid>();

                    TraceHelper.Trace(tracingService, "Batching IDs received: {0}", batchingIds.Count);


                    // Take ALL IDs 
                    var currentBatchIds = batchingIds.ToList();

                   

                    TraceHelper.Trace(tracingService, "Processing batch count: {0}", currentBatchIds.Count);


                    //Retrieve ONLY those 8 OLIs
                    List<Entity> sourceProducts = new List<Entity>();
                    ...
                    if (currentBatchIds.Count > 0)
                    {
                                ...
                                Conditions =
                                            {
                                                new ConditionExpression(
                                                    ...
                                                    currentBatchIds.Cast<object>().ToArray()
                                                )
                                            }
                            }
                        };
                        ...
                        sourceProducts = result.Entities.ToList();
                    }

                    //Rebuild copied OLIs
                    List<Entity> BatchingProductsToCopy = new List<Entity>();
                    ...
                    foreach (var product in sourceProducts)
                    {
                        ...
                        foreach (var attr in product.Attributes)
                        {
                            ...
                            if (key.Equals("opportunityid", StringComparison.OrdinalIgnoreCase) ||
                                key.Equals("statecode", StringComparison.OrdinalIgnoreCase) ||
                            ...
                            // your special logic
                            if (key.Equals("ats_sellingrate", StringComparison.OrdinalIgnoreCase))
                            {
                                Money ppu = product.GetAttributeValue<Money>("priceperunit");
                                newProduct[key] = ppu ?? attr.Value;
                                continue;
                            }

                            newProduct[key] = attr.Value;
                        }

                        // keep source id for reference
                        //newProduct["ats_sourceopportunityproductid"] = product.Id.ToString();
                        newProduct.Id = product.Id; 
                        BatchingProductsToCopy.Add(newProduct);
                    }


                    Guid opportunityId = Guid.TryParse(pluginContext.InputParameters.Contains("InputOpportunityId") ? pluginContext.InputParameters["InputOpportunityId"]?.ToString() : null,out Guid oppId) ? oppId : Guid.Empty;

                    Guid seasonId = Guid.TryParse(pluginContext.InputParameters.Contains("InputSeasonId") ? pluginContext.InputParameters["InputSeasonId"]?.ToString() : null,out Guid sId) ? sId : Guid.Empty;

                    Guid lastSeasonYearId = Guid.TryParse(pluginContext.InputParameters.Contains("InpuLastSeasonYear") ? pluginContext.InputParameters["InpuLastSeasonYear"]?.ToString() : null,out Guid lId) ? lId : Guid.Empty;

                    string jsonPendingPackageLinks = pluginContext.InputParameters.Contains("PendingPackageLinks") ? pluginContext.InputParameters["PendingPackageLinks"]?.ToString(): null;

                    List<PendingPackageLink> pendingPackageLinks = string.IsNullOrWhiteSpace(jsonPendingPackageLinks) ? new List<PendingPackageLink>()
                        : JsonConvert.DeserializeObject<List<PendingPackageLink>>(jsonPendingPackageLinks);


                    TraceHelper.Trace(tracingService, "CustomAPI Inputs | OppId={0} | SeasonId={1} | LastSeasonYearId={2} | PendingLinksCount={3}", opportunityId, seasonId, lastSeasonYearId, pendingPackageLinks?.Count ?? 0);


                    List<Entity> productsToCopyPassNull = null;
                    bool isBatcing = true;
                    bool isOLIBatchingNeeded = false;
                    //Associating the opportunity products to the opportunity
                    ProductsToCopyStage(service, tracingService, getOpportunityProducts, productsToCopyPassNull, ref BatchingProductsToCopy, opportunityId, seasonId, lastSeasonYearId, isBatcing, pluginContext, ref pendingPackageLinks, ref isOLIBatchingNeeded);

                    if (!isOLIBatchingNeeded) //No more OLI batching is remaining..
                    {
                        UpdatePendingPackageLink(service, tracingService, pendingPackageLinks, opportunityId, seasonId);
                    }
                ...
                var agreementRef = (EntityReference)
      pluginContext.InputParameters["ats_agreemententityreference"];
                ...
                var batchCountRef = (int)
                    pluginContext.InputParameters["ats_batchCount"];
                ...
                if (inputActionName == "CalculateTotalDealValueAgreement")
                {
                    ...
                    if (pluginContext != null) pluginContext.OutputParameters["ats_iscreateopportunitybatching"] = "false";
                ...
                if (contractLength != sortedOpportunityCollection.Count)
                {
                    ...
                    if (oppsToDelete.Count > 0)
                    {
                        ...
                        if (pluginContext != null) pluginContext.OutputParameters["ats_isDeleteOpportunity"] = true;
         ...
         lastSeasonOpportunity.GetAttributeValue<EntityReference>("ats_startseason");
                ...
                foreach (var season in seasonDict.OrderBy(s => s.Key))
                {
                    ...
                    //Sunny(3-3-26) --> copying the product into another list
                    List<Entity> BatchingProductsToCopy = new List<Entity>(productsToCopy);
                    TraceHelper.Trace(tracingService, "BatchingProductsToCopy: {0}", BatchingProductsToCopy.Count); 
                    ...
                    if (productsToCopy != null && productsToCopy.Count > 0)
                    {
                        ...
                        List<PendingPackageLink> pendingPackageLinks = new List<PendingPackageLink>();
                        bool isBatching = false;
                        bool isOLIBatchingNeeded = false; 
                        //Associating the opportunity products to the opportunity
                        ProductsToCopyStage(service, tracingService, getOpportunityProducts, productsToCopy, ref BatchingProductsToCopy, newOpportunityId, seasonEntity.Id, lastSeasonId, isBatching, pluginContext , ref pendingPackageLinks, ref isOLIBatchingNeeded);

                        if (!isOLIBatchingNeeded)
                        {
                            UpdatePendingPackageLink(service, tracingService, pendingPackageLinks, newOpportunityId, seasonEntity.Id);
                        }
                    ...
                    if (inputActionName == "NewAgreementrecordBulkCreateOpp")
                    {
                        TraceHelper.Trace(tracingService, "inputActionName == 'NewAgreementrecordBulkCreateOpp'");
                        // Your existing behavior: only create FIVE opportunity per run
                        if (opportunitiesCreated == batchCountRef)
                        {
                    ...
                    {
                        TraceHelper.Trace(tracingService, "inputActionName is not 'NewAgreementrecordBulkCreateOpp', so continuing to create opportunities for all seasons. Current count={0}", opportunitiesCreated);
                ...
                if (opportunityCollectionUpdated.Entities.Count == seasonDict.Count)
                {
                    ...
                    if (pluginContext != null) pluginContext.OutputParameters["ats_iscreateopportunitybatching"] = false;
                    TraceHelper.Trace(tracingService, "ats_iscreateopportunitybatching kept true.");
                    pluginContext.OutputParameters["OpportunityEntityReference"] =
                       lastTouchedOppId == Guid.Empty
                           ? null
                           : new EntityReference("opportunity", lastTouchedOppId);
                    TraceHelper.Trace(tracingService, "Opp Entity reference is passed...");
                }
                else
                {
                    // if (context != null) IsCreateOpportunityBatching.Set(context, hasMissingOpps ? "true" : "false");
                    //if (pluginContext != null) pluginContext.OutputParameters["ats_iscreateopportunitybatching"] = hasMissingOpps ? true : false;

                    if (pluginContext != null) pluginContext.OutputParameters["ats_iscreateopportunitybatching"] = true;
                    ...
                    if (pluginContext == null)
                        throw new InvalidPluginExecutionException("Plugin context is null.");

                    // 1️⃣ EntityReference output
                    pluginContext.OutputParameters["OpportunityEntityReference"] =
                        lastTouchedOppId == Guid.Empty
                            ? null
                            : new EntityReference("opportunity", lastTouchedOppId);
                    ...
                    // 2️ String output
                    pluginContext.OutputParameters["Action"] = "ReCalcOppLinesAgreement";

                    // 3️ Boolean output
                    pluginContext.OutputParameters["isFromBatching"] = true;

                    // 4️ Boolean output
                    pluginContext.OutputParameters["isDeleteOpportunity"] = false;


                    TraceHelper.Trace(tracingService, "Done. lastTouchedOppId={0}", lastTouchedOppId
                );
        ...
        private static void UpdatePendingPackageLink(IOrganizationService service, ITracingService tracing, List<PendingPackageLink> pendingPackageLinks, Guid newOpportunityId, Guid newSeasonId)
        {
            string functionName = "UpdatePendingPackageLink";
            try
            {
        ...
        private void UpdateRollUpHandlePostBatching(ITracingService tracingService)
        {
            string functionName = "UpdateRollUpHandlePostBatching"; 
            try
            {
                TraceHelper.Trace(tracingService, ""); 
            }
        ...
        List<Entity> BatchingProductsToCopy,
        ...
        Guid lastSeasonId)
        {
            ...
            foreach (var source in productsToCopy)
            {
                ...
                // Helpful to trace source identity “which line caused issue”
        ...
        private static void ProductsToCopyStage(
    IOrganizationService service,
    ITracingService tracing,
    GetOpportunityProducts helper,
    List<Entity> productsToCopy,
    ref List<Entity> BatchingProductsToCopy, // passed as an output in the script, and used for olis batching
    Guid newOpportunityId,
    Guid newSeasonId,
    Guid lastSeasonId,
    bool isBatching,
    IPluginExecutionContext pluginContext,
    ref List<PendingPackageLink> pendingPackageLinks,
    ref bool isOliBatchingNeeded)
        {
            string functionName = "ProductsToCopyStage";
            try
            {
                // Defensive traces for critical inputs (common break points)
                TraceHelper.Trace(tracing, "CopyProductsToNewOpportunity_OptimizedNoBatch START");
                ...
                if (productsToCopy == null && !isBatching)
                {
                    TraceHelper.Trace(tracing, "ERROR: productsToCopy is null. Exiting method.");
                    return;
                }

                if (pendingPackageLinks == null)
                {
                    pendingPackageLinks = new List<PendingPackageLink>();
                }
                ...
                if (BatchingProductsToCopy == null || BatchingProductsToCopy.Count == 0)
                {
                    TraceHelper.Trace(tracing, "BatchingProductsToCopy is null or empty. Exiting.");
                    return;
                }

                TraceHelper.Trace(tracing, "BatchingProductsToCopy.Count: {0}", BatchingProductsToCopy.Count);




                int batchSize = 20;
                int limit = Math.Min(batchSize, BatchingProductsToCopy.Count);


                TraceHelper.Trace(tracing, "limit value: {0}", limit);
                ...
                for (int i = 0; i < limit; i++)
                {
                    ...
                    if (BatchingProductsToCopy == null || BatchingProductsToCopy.Count == 0)
                    {
                        TraceHelper.Trace(tracing, "BatchingProductsToCopy became empty during loop. Breaking.");
                        break;
                    }

                    // Always process the first item because the list is being reduced after create
                    Entity source = BatchingProductsToCopy[0];

                    if (source == null)
                    {
                        TraceHelper.Trace(tracing, "WARN-Break: source entity is null at index={0}. Breaking.", sourceIndex);
                        break;
                    }
                    ...
                    if (target == null)
                    {
                        ...
                        // remove current bad record so loop does not get stuck on same item
                        BatchingProductsToCopy.RemoveAt(0);
                        continue;
                    }
                    ...
                    // Ensure InventoryBySeason for this season/product
                    EntityReference prodRef = source.GetAttributeValue<EntityReference>("productid");
                    int prodQty = source.Contains("quantity") && source["quantity"] != null
                        ? Convert.ToInt32(source["quantity"])
                        : 0;
                    ...
                    catch (Exception createEx)
                    {
                        ...
                        // remove failed current item also, so batching can move ahead
                        BatchingProductsToCopy.RemoveAt(0);
                        continue;
                    }
                    ...
                    // remove the processed item safely from the front
                    BatchingProductsToCopy.RemoveAt(0);
                }


                TraceHelper.Trace(tracing, "BatchingProductsToCopy Updated Count: {0}", BatchingProductsToCopy.Count);



                if (BatchingProductsToCopy.Count > 0) // passing the output parameter back to the script for next batch, so we exit this method after processing the batch and pass the remaining products back to script via output parameter
                {
                    TraceHelper.Trace(tracing, "OLi Batching is remaining.");

                    //string jsonBatchingProductsToCopy = JsonConvert.SerializeObject(BatchingProductsToCopy);
                    //pluginContext.OutputParameters["BatchingProductsToCopy"] = jsonBatchingProductsToCopy;
                    var remainingIds = BatchingProductsToCopy
                                        .Where(x => x != null && x.Id != Guid.Empty)
                                        .Select(x => x.Id)
                                        .ToList();

                    string jsonBatchingProductsToCopy = JsonConvert.SerializeObject(remainingIds);
                    pluginContext.OutputParameters["BatchingProductsToCopy"] = jsonBatchingProductsToCopy;

                    TraceHelper.Trace(tracing, "Remaining batching IDs count: {0}", remainingIds.Count);

                    string jsonPendingPackageLink = JsonConvert.SerializeObject(pendingPackageLinks);
                    pluginContext.OutputParameters["OLIPendingPackageLinks"] = jsonPendingPackageLink;
                    pluginContext.OutputParameters["isOLIBatching"] = true;
                    pluginContext.OutputParameters["OLIBatchingOpportunityId"] = newOpportunityId.ToString();
                    pluginContext.OutputParameters["OLIBatchingOpportunityNewSeasonId"] = newSeasonId.ToString();
                    pluginContext.OutputParameters["OLIBatchingOpportunityLastSeasonId"] = lastSeasonId.ToString();
                    isOliBatchingNeeded = true;
                    TraceHelper.Trace(tracing, "BatchingProductsToCopy current count: {0}", BatchingProductsToCopy.Count);
                    return;
                }
                else // pending package links, seasonentity Id, New opportunity Id
                {
                    TraceHelper.Trace(tracing, "No mor OLIs are remaining");
                    string jsonPendingPackageLink = JsonConvert.SerializeObject(pendingPackageLinks);
                    pluginContext.OutputParameters["OLIPendingPackageLinks"] = jsonPendingPackageLink;
                    pluginContext.OutputParameters["OLIBatchingOpportunityId"] = newOpportunityId.ToString();
                    pluginContext.OutputParameters["OLIBatchingOpportunityNewSeasonId"] = newSeasonId.ToString();
                    pluginContext.OutputParameters["isOLIBatching"] = false; // no more batching needed, we can proceed to link the package lines in the same transaction
                    isOliBatchingNeeded = false;
                }
            ...
            {
                throw new InvalidPluginExecutionException($"functionName: {functionName}, Exception: {pex.Message}");
            }
            catch (Exception ex)
            {
                throw new InvalidPluginExecutionException($"functionName: {functionName}, Exception: {ex.Message}");
        ...
        // Adjust this FetchXML if your "package line" identification needs extra condition.
        ...
        private static Entity CloneOpportunityProductForCreate(Entity source)
        {
            ...
            foreach (var kv in source.Attributes)
            {
                ...
                target.Attributes[k] = kv.Value; //Cloning the opp prod
            }
