...
{
    public class AgreementPaymentScheduleBusinessLogic
    {
    ...
    <filter>
      <condition attribute='ats_scheduledpaymentid' operator='eq' value='{0}' />
        ...
        public void CreatePaymentSchedule(CodeActivityContext context, Dictionary<string, string> args)
        {
            ...
            Guid opportunityId = Guid.Parse(args["OpportunityId"]);
            #region logic fro retrieving the agreement id from the opportunityid 
            Entity oppData = service.Retrieve("opportunity", opportunityId, new ColumnSet("ats_agreement"));

            Guid agreementId = Guid.Empty;
            if (oppData.Attributes.Contains("ats_agreement") && oppData["ats_agreement"] != null)
            {
                tracingService.Trace("Agreement id is found");
                EntityReference agreementRef = (EntityReference)oppData["ats_agreement"];
                agreementId = agreementRef.Id;
                tracingService.Trace($"Agreement Id: {agreementId}");
                ScheduledPayment["ats_agreementid"] = new EntityReference("ats_agreement", agreementId);
            }
            #endregion


            ScheduledPayment["ats_opportunityid"] = new EntityReference("opportunity", Guid.Parse(args["OpportunityId"]));
            //ScheduledPayment["ats_agreementid"] = new EntityReference("ats_agreement", Guid.Parse(args["AgreementId"]));
        ...
        public void UpdatePaymentSchedule(CodeActivityContext context, Dictionary<string, string> args)
        {
            ...
            foreach (var scheduleDetail in retrievedSchedules.Entities)
            {
                ...
                //scheduleToUpdate["ats_agreementid"] = new EntityReference("ats_agreement", Guid.Parse(args["AgreementId"]));
        ...
        public void DeletePaymentSchedule(CodeActivityContext context, Dictionary<string, string> args)
        {
            ...
            tracingService.Trace($"Inside Delete Payment Schedule****** ID:  {args["PaymentScheduleId"].ToString()}"); 
            ...
            tracingService.Trace("END OF Delete Payment Schedule******"); 
